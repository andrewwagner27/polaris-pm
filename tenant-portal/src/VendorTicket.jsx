...
export default function VendorTicket() {
  ...
  async function markComplete() {
    if (!ticket) return;
    setCompleting(true);
    await supabase.from("maintenance_requests").update({ status: "resolved" }).eq("id", ticket.id);
    await supabase.from("maintenance_comments").insert({
      request_id:  ticket.id,
      body:        `${vendorName} marked this ticket as complete.`,
      author_type: "vendor",
      author_name: vendorName,
    ...
    <div style={{ minHeight:"100vh", background:C.bg, color:C.text, fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"24px 20px 80px" }}>
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, overflow:"hidden", marginBottom:16 }}>
          ...
          {photos.length === 0 ? (
            <div style={{ padding:"20px", textAlign:"center", color:C.textMuted, fontSize:13 }}>No photos yet.</div>
          ) : (
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, overflow:"hidden", marginBottom:16 }}>
          <div style={{ padding:"14px 16px", borderBottom:`1px solid ${C.border}` }}>
            ...
            <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>Your comments are visible to the property manager only</div>
          </div>
        ...
        {/* Mark complete */}
        {!isResolved && (
          <button onClick={markComplete} disabled={completing} style={{ width:"100%", padding:"13px", background:"rgba(114,176,42,0.1)", border:`1px solid rgba(114,176,42,0.25)`, borderRadius:8, fontSize:14, fontWeight:500, color:C.green, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"all 0.15s" }}>
            {completing ? <><Spinner/> Marking complete…</> : "✓ Mark work as complete"}
          </button>
        )}
