    ...
    <div style={{ width:"100%",fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ flex:1,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"0 24px 40px" }}>
        <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:"0 0 14px 14px",width:"100%",maxWidth:420,padding:"28px 28px 32px",borderTop:"none" }}>
          ...
          {/* Sign in form */}
          <div style={{ fontSize:16,fontWeight:600,color:C.text,marginBottom:20 }}>Sign in</div>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{ width:"100%",padding:"12px",border:`1px solid ${C.goldDim}`,borderRadius:8,fontSize:14,fontWeight:500,background:loading?"rgba(201,169,110,0.07)":"transparent",color:C.gold,cursor:loading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"'DM Sans',sans-serif",transition:"background 0.15s",opacity:loading?0.7:1 }}
          ...
          {/* Invite only notice */}
          <div style={{ marginTop:24,padding:"14px 16px",background:C.raised,border:`1px solid ${C.border}`,borderRadius:8,display:"flex",alignItems:"flex-start",gap:10 }}>
            <div style={{ width:6,height:6,borderRadius:"50%",background:C.goldDim,flexShrink:0,marginTop:5 }}/>
            <div>
              <div style={{ fontSize:12,fontWeight:500,color:C.textSub,marginBottom:2 }}>Access by invite only</div>
              <div style={{ fontSize:11,color:C.textMuted,lineHeight:1.5 }}>
                Don't have an account? Contact your property manager to receive an invitation.
              </div>
            </div>
          </div>
