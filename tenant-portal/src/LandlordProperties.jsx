...
function PrimaryBtn({ children, onClick, small }) {
  return (
    <button onClick={onClick} style={{ background:"transparent",border:`1px solid ${C.goldDim}`,color:C.gold,fontSize:small?11:13,fontWeight:500,padding:small?"6px 12px":"9px 18px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",letterSpacing:"0.04em",transition:"background 0.15s" }}
...
function GhostBtn({ children, onClick, small }) {
  return (
    <button onClick={onClick} style={{ background:"transparent",border:`1px solid ${C.border}`,color:C.textSub,fontSize:small?11:13,fontWeight:500,padding:small?"6px 12px":"9px 18px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all 0.15s" }}
...
function AddPropertyModal({ onClose, onSaved }) {
  const [form, setForm] = useState({ name:"", address:"", city:"", state:"", zip:"", property_code:"" });
  ...
  async function save() {
    ...
    if (!form.property_code.trim()) { setError("Property code is required."); return; }
    setSaving(true);
    const { error } = await supabase.from("properties").insert({
      ...
      state:         form.state,
      zip:           form.zip,
      property_code: form.property_code.toUpperCase().trim(),
    });
    setSaving(false);
    if (error) {
      if (error.message.includes("unique")) {
        setError("That property code is already in use. Choose a different one.");
      } else {
      ...
      <div style={{ background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,width:480,maxHeight:"90vh",overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"20px 24px" }}>
          ...
          <div style={{ marginBottom:6 }}>
            ...
            <Input value={form.property_code} onChange={e=>update("property_code",e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,""))} placeholder="e.g. CLIFTON" maxLength={10}/>
          </div>
          <div style={{ fontSize:11,color:C.textMuted,marginBottom:14,lineHeight:1.5 }}>
            Tenants enter this code during onboarding to link to this property. Use a short, memorable all-caps word (letters and numbers only). Must be unique.
          </div>
        </div>
                  ...
                  <div style={{ padding:"16px 18px" }}>
                    <div style={{ display:"flex",alignItems:"flex-start",gap:10,marginBottom:12 }}>
                      ...
                      <div>
                        ...
                        {prop.property_code&&(
                          <div style={{ display:"inline-flex",alignItems:"center",gap:4,marginTop:4,padding:"2px 8px",background:`${C.gold}12`,border:`1px solid ${C.goldDim}`,borderRadius:4 }}>
                            <span style={{ fontSize:9,fontWeight:700,color:C.textMuted,letterSpacing:"0.08em" }}>CODE</span>
                            <span style={{ fontSize:11,fontWeight:700,color:C.gold,letterSpacing:"0.1em" }}>{prop.property_code}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ display:"flex",gap:8,paddingTop:12,borderTop:`1px solid ${C.border}` }}>
                      <button onClick={e=>{ e.stopPropagation(); setSelected(prop.id); setDetailTab("Units"); }} style={{ flex:1,padding:"7px 0",background:"transparent",border:`1px solid ${C.goldDim}`,borderRadius:6,fontSize:11,fontWeight:500,color:C.gold,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"background 0.15s" }}>View units</button>
                      <button onClick={e=>{ e.stopPropagation(); navigate("/landlord/maintenance"); }} style={{ flex:1,padding:"7px 0",background:"transparent",border:`1px solid ${C.border}`,borderRadius:6,fontSize:11,fontWeight:500,color:C.textSub,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"all 0.15s" }}>Maintenance</button>
                    </div>
                  </div>
                </div>
              );
          ...
          <div style={{ width:540,background:C.surface,height:"100vh",overflowY:"auto",borderLeft:`1px solid ${C.border}` }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ padding:"20px 24px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"flex-start",justifyContent:"space-between" }}>
              <div>
                <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:4 }}>
                  ...
                  {prop.property_code&&(
                    <span style={{ fontSize:10,fontWeight:700,padding:"2px 8px",background:`${C.gold}12`,border:`1px solid ${C.goldDim}`,borderRadius:4,color:C.gold,letterSpacing:"0.08em" }}>{prop.property_code}</span>
                  )}
                </div>
            ...
            <div style={{ display:"flex",borderBottom:`1px solid ${C.border}`,padding:"0 24px" }}>
              {["Overview","Units"].map(tab=>(
                <button key={tab} onClick={()=>setDetailTab(tab)} style={{ padding:"11px 16px",fontSize:13,fontWeight:detailTab===tab?600:400,color:detailTab===tab?C.gold:C.textSub,background:"none",border:"none",borderBottom:detailTab===tab?`2px solid ${C.gold}`:"2px solid transparent",marginBottom:-1,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"color 0.15s" }}>{tab}</button>
              ))}
            </div>
                ...
                <>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20 }}>
                    {[
                      ...
                      ["Property code", prop.property_code||"Not set"],
                    ].map(([k,v])=>(
                      <div key={k} style={{ background:C.raised,borderRadius:8,padding:"11px 14px" }}>
                        ...
                        <div style={{ fontSize:15,fontWeight:600,color:k==="Property code"?C.gold:C.text }}>{v}</div>
                      </div>
                    ))}
                  </div>
