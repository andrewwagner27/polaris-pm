...
function DangerBtn({ children, onClick }) {
  return (
    <button onClick={onClick}
      style={{ background:"rgba(224,85,85,0.1)",border:`1px solid rgba(224,85,85,0.25)`,color:C.red,fontSize:13,fontWeight:500,padding:"9px 18px",borderRadius:7,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap" }}
    >{children}</button>
  );
}
...
export default function LandlordApplications() {
  ...
  const [form, setForm] = useState({
    ...
    move_in_date:"", monthly_income:"", notes:""
  });
  ...
  async function handleUploadReport(appId) {
    ...
    if (dbErr) { showToast("Failed to save report", true); setUploadingReport(false); return; }
  ...
  async function sendInviteForTenant(tenantId, app, unit, property) {
    const { error: inviteErr } = await supabase.functions.invoke("invite-tenant", {
      body: {
        tenant_id:     tenantId,
  ...
  async function handleApprove(app) {
    ...
    // Send invite
    if (app.email) {
      const inviteErr = await sendInviteForTenant(tenant.id, app, unit, property);
      if (inviteErr) {
        showToast(`Approved ✓ — invite failed. Use the Resend invite button below.`, true);
      } else {
        showToast(`${app.name} approved and invited to portal`);
      }
    } else {
      showToast(`${app.name} approved — no email on file, add one to send invite`);
    }

    setConverting(false);
    // Refresh selected with updated status
    const updatedApp = { ...app, status: "approved", decided_at: new Date().toISOString() };
    setSelected(updatedApp);
    setApplications(prev => prev.map(a => a.id === app.id ? updatedApp : a));
  }

  async function handleResendInvite(app) {
    if (!app.email) { showToast("No email on file", true); return; }
    setSendingInvite(true);

    // Find the tenant record created from this application
    const { data: tenantData } = await supabase.from("tenants").select("id, unit_id").eq("email", app.email).single();
    if (!tenantData) {
      showToast("Tenant record not found — add them manually in Tenants", true);
      setSendingInvite(false);
      return;
    }

    const unit     = units.find(u => u.id === (tenantData.unit_id || app.unit_id));
    const property = properties.find(p => p.id === app.property_id);
    const inviteErr = await sendInviteForTenant(tenantData.id, app, unit, property);

    setSendingInvite(false);
    if (inviteErr) {
      showToast(`Invite failed: ${inviteErr.message}`, true);
    } else {
      showToast(`Invite resent to ${app.email}`);
    }
  }
              ...
              <div style="background:#0A0B0D;padding:32px 20px;font-family:'Helvetica Neue',sans-serif;">
                <div style="background:#111316;border:1px solid #252930;border-radius:12px;max-width:560px;margin:0 auto;overflow:hidden;">
                  ...
                  <div style="padding:16px 32px;border-top:1px solid #252930;">
                    <p style="margin:0;font-size:11px;color:#5C6270;">Modus Property Management</p>
                  </div>
                </div>
              </div>
            `,
  ...
  return (
    <LandlordLayout openMaintenance={0} unreadMessages={0}>
      ...
      {toast&&(
        <div style={{ position:"fixed",top:20,right:20,zIndex:300,background:toast.isError?C.red:C.green,color:"#fff",padding:"10px 18px",borderRadius:8,fontSize:13,fontWeight:500,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 16px rgba(0,0,0,0.4)",maxWidth:360 }}>
          {toast.msg}
        </div>
      )}
            ...
            <div style={{ padding:"20px 24px" }}>
              ...
              {selected.status==="pending"&&(
                <div style={{ display:"flex",gap:10,marginTop:8 }}>
                  <button onClick={()=>handleApprove(selected)} disabled={converting}
                    ...
                    {converting?"Processing…":"✓ Approve & invite to portal"}
                  </button>
              ...
              {selected.status==="approved"&&(
                <div style={{ display:"flex",flexDirection:"column",gap:10,marginTop:8 }}>
                  <div style={{ padding:"12px 14px",background:`${C.green}10`,border:`1px solid ${C.green}25`,borderRadius:8,fontSize:13,color:C.green,textAlign:"center" }}>
                    ✓ Approved — tenant record created
                  </div>
                  <button onClick={()=>handleResendInvite(selected)} disabled={sendingInvite}
                    style={{ width:"100%",padding:"11px",background:"transparent",border:`1px solid ${C.goldDim}`,borderRadius:8,fontSize:13,fontWeight:500,color:C.gold,cursor:sendingInvite?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif",opacity:sendingInvite?0.7:1 }}>
                    {sendingInvite?"Sending…":"↺ Resend portal invite"}
                  </button>
                </div>
              )}
            </div>
